...
-- MySQL 은 DB 와 관련된 명령을 실행할때
-- 가장먼저 어떤DB 를 사용할 것인지 알려줘야 한다

USE myDB;
DROP TABLE tbl_test;
-- Table 을 생성할때 칼럼의 이름은 사전에 있는 키워드는 가급적
-- 사용하지 않는것이 좋다
-- 사전에 있는 키워드를 사용할때는
-- 접두사를 부착하여 사용하는 것이 좋다.  
...
/*
 ...
 사용자는 Database 를 사용하겠다 라고 선언을 한 후
 DB 관련된 명령을 사용한다
*/
...
/*
 ...
 데이터를 INSERT 할때마다 자동으로 Sequence 값이 생성되어
 추가된다
 5.x MySQL 에서는 AUTO_INCREMENT 칼럼에 이슈가 있었다
 5.x 숫자 type 은 주로 INT 형을 사용하는데 INT 형은 정수 11자리이다
 때문에 정수형 11자리 범위를 벗어나는 경우 데이터 INSERT 가 불가능해지는
 문제가 있었다
 5.7 버전 이후에는 BIGINT 형 타입이 도입되어 문제를 해결하였다
 DB table 을 만들때 AUTO_INCREMENT 칼럼에는 반드시 BIGINT 형을 지정하자
 ...
 BIGINT : 8Byte : -2의 7승 -1부터 2의 7승 까지
*/
/*

AUTO_INCREMENT 칼럼을 만들때 주의할점
데이터 타입은 BIGINT 로 설정
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','김','군',1,'010101','박');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0303','김씨','과과',3,'01010101','광주광역시tbl_ems이');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0102','ì´ëª½ë£¡','ì»´í°',3,'010-10100','ê´ì£¼ê´ì­ì');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0103','이몽룡','강컹',3,'0101010','광주시아');
