...
과목수가 늘어나면 학번, 과목명, 점수 형식으로 계속 저장해준다

설계 필요한 데이터는 학번, 과목명, 점수 칼럼인데, PK 를 설정하기 위해 SEQ 라는
칼럼이 추가된 상태

SEQ 칼럼이 없이 PK를 설정하려면 학번 + 과목명으로 두개의 칼럼을 묶어서
PK로 해야한다.
다수의 칼럼을 묶음으로 PK를 설정하는 것을 Super Key 라고 한다.
필요에 따라 Super Key로 PK 를 설정 하기도 하지만 Super Key 는 잘못 사용하면
데이터 무결성을 해칠 수 있다.
...
-- UPDATE 나 DELETE 를 실행할때 조건절(WHERE)에 복수의 칼럼 조건을 부여하는 것은
-- 썩 좋지않다.
UPDATE tbl_score SET sc_score = 80
WHERE sc_stnum = 'S0044' AND sc_subject = '미술';

-- 1. 변경하고자 하는 점수의 데이터를 먼조 조회한다
...
-- 2. tbl_score 칼럼과 table 칼럼 데이터를 보여주면서
-- 3. tbl_score.sc_num 데이터를 가지고 tbl_student.st_num 를 SELECT 한 후
-- 4. 일치하는 데이터가 있으면 st_name칼럼을 가져와서 함께보여라
