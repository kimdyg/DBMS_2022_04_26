...
-- 학생의 학과 소속 정보 Relation 을 Table로 생성
-- Relation 을 Table 로 설계할때 가장 유의해야 할 부분
-- Entity table 의 칼럼과 DOMAIN(type 과 길이) 을 반드시 일치시켜야한다

-- 복수의 칼럼이 UNIQUE, NOT NULL 선언되었다
-- 이러한 경우 복수의 칼럼을 묶어 PK로 선언하기도한다.
-- 이 Table 에 INSERT 수행할 때는 데이터 검증이 잘되어
-- 잘못된 데이터가 추가되는것을 방지할 수 있다
...
-- 전체 칼럼에 데이터를 포함한 INSERT 할때는 칼럼리스트를 생략할 수 있다
...
INSERT INTO tbl_belong(b_seq,b_stnum, b_dcode) VALUES('20220002','D0001');
