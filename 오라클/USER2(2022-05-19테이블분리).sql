...
/*
성적 데이터가 저장된 tbl_score table의 데이터중
영어 과목을 영어 영문으로 변경하고 싶다
SQL UPDATE 명령을 이용하면 간단히 해결할 수 있따
...
1. tbl_score 에 sc_sbcode "칼럼을 추가"하기
...
-- tbl_ score 테이블에 update 실행하기

-- 아래 명령은 전체 데이터의 sc_sbcode 칼럼의 값을 001 문자열로 대체한다
-- 드디어 일주일간 야근이다
-- 경고 : UPDATE와 DELETE 명령은 WHERE 이 없을때 절대 실행 금지~~!!
...
-- 학번과 점수는 있는데 과목코드가 없으면 보유한 데이터에 문제가 있을 수 있다
-- 또한 점수를 입혁하지 않은 것으로 생각하고
-- 반복하여 값을 저장할 수 있다
--새로 생성한 칼럼을 NOT NULL 로 설정한다
-- NOT NULL 제약 조건: 값을 INSERT 할때 이 칼럼은 비워 놓아서는 안된다
ALTER TABLE tbl_score MODIFY (sc_sbcode NOT NULL);
-- 이후 부터 데이터를 INSERT 할때 sc_sbcode 칼럼의 데이터가 없으면 INSERT 가 거부된다
...
SELECT
    COUNT(*) FROM tbl_score, tbl_subject;
